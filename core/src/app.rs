...
use crate::{p2p::{build_swarm, load_or_generate_keypair, AgentBehaviour, AgentEvent}, state::{Connection, State}};

pub enum Command {
    Connect { peer_id: libp2p::PeerId, addr: libp2p::Multiaddr },
}
    ...
    async fn handle_swarm_event(&mut self, event: SwarmEvent<AgentEvent>) {
        match event {
            ...
            SwarmEvent::ConnectionEstablished { peer_id, connection_id, endpoint, num_established, concurrent_dial_errors, established_in } => {
                self.state.connections.push(Connection { peer_id, connection_id });
            },
            SwarmEvent::ConnectionClosed { peer_id, connection_id, endpoint, num_established, cause } => {
                self.state.connections.retain(|c| c.connection_id != connection_id);
            },
            SwarmEvent::IncomingConnection { connection_id, local_addr, send_back_addr } => todo!(),
            SwarmEvent::IncomingConnectionError { connection_id, local_addr, send_back_addr, error, peer_id } => todo!(),
